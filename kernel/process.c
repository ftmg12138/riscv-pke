    ...
    switch( parent->mapped_info[i].seg_type ){
      ...
      case CODE_SEGMENT:
        ...
        for(int j=0;j<parent->mapped_info[i].npages;j++){
          uint64 pa_map_va = lookup_pa(parent->pagetable, parent->mapped_info[i].va + j * PGSIZE);
          map_pages(child->pagetable, parent->mapped_info[i].va + j * PGSIZE, PGSIZE, pa_map_va, prot_to_type(PROT_READ | PROT_EXEC, 1));// * 权限为可读、可执行
        } 
